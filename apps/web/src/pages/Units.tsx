    ...
    <div className="p-4 md:p-8 max-w-7xl mx-auto space-y-6 h-full flex flex-col animate-fade-in">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 shrink-0">
        <div className="bg-white rounded-xl border border-slate-100 p-4 shadow-sm">
          <p className="text-xs text-slate-500 font-medium">Total Unidades</p>
          <p className="text-2xl font-bold text-slate-900 mt-1">{units.length}</p>
        </div>
        <div className="bg-white rounded-xl border border-slate-100 p-4 shadow-sm">
          <p className="text-xs text-slate-500 font-medium">Disponibles</p>
          <p className="text-2xl font-bold text-emerald-600 mt-1">{units.filter(u => u.status === 'DISPONIBLE').length}</p>
        </div>
        <div className="bg-white rounded-xl border border-slate-100 p-4 shadow-sm">
          <p className="text-xs text-slate-500 font-medium">Separados / Reservados</p>
          <p className="text-2xl font-bold text-purple-600 mt-1">{units.filter(u => u.status === 'SEPARADO' || u.status === 'RESERVADO').length}</p>
        </div>
        <div className="bg-white rounded-xl border border-slate-100 p-4 shadow-sm">
          <p className="text-xs text-slate-500 font-medium font-sans">Vendidos</p>
          <p className="text-2xl font-bold text-amber-600 mt-1">{units.filter(u => u.status === 'VENDIDO').length}</p>
        </div>
      </div>
      ...
      <div className="flex-1 flex gap-6 overflow-hidden min-h-0">
        {/* Units Table */}
        <div className={`bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden flex flex-col ${selectedUnit ? 'w-1/2' : 'w-full'} transition-all duration-300`}>
